  ...
  public void execute(List<CachableRenderStatement> csr, boolean directDrawingEnabled) {
    if (!derpedTess && directDrawingEnabled && glErrorCount != -1) {
      ...
      if (hasGLerrors() && glErrorCount++ > 1000) {
        glErrorCount = -1;
        GravityTorch.LOG
            .warn("Disabled direct drawing after too many OpenGL errors. If you got rendering errors, you can"
                + " disabled direct drawing completely in the mod options. Last error was: " + lastError + " ("
  ...
  public static void setupTessellatorFields() {
    if (!derpedTess) {
      try {
        ...
        GravityTorch.LOG.warn("Fields available:" + b);
        derpedTess = true;
      }
    }
  }
